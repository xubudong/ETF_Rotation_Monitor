...
- `portfolio.py`：读取 `web_app/data/portfolio_industries.json`，提供组合诊断。
...
`A 股板块轮动` 和 `全球与大宗配置` 是两个独立账户，可分别用页面上的持仓按钮上传券商导出的 `table.xls`。上传后各自页面只高亮自己的持仓，`活跃科技标的` 页面显示两个账户持仓并集。排名表中的 `Top12` 列会标出相对上一交易日的新进前 12 和掉出前 12。

页面会常驻显示两个账户的今日涨跌幅。计算方式优先使用已上传持仓里的实际数量：`今日盈亏 = 持仓数量 × (实时最新价 - 昨收价)`，账户涨跌幅按全部持仓的今日盈亏和昨日市值汇总。单标的价格优先来自当前选择的实时数据源；实时缺失时回退到本地历史库最后两根 K 线。这样上传一次持仓后，后续刷新排名或切换数据源即可更新账户涨跌。

排名表还会显示 `昨日排名` 与 `排名变化`，用于快速判断相对上一交易日的升降位。

上传持仓后，页面的 `组合诊断` 面板会按 `web_app/data/portfolio_industries.json` 里的 ETF 行业穿透配置汇总申万一级/二级行业暴露，支持 A 股账户、全球账户和全部账户切换。
